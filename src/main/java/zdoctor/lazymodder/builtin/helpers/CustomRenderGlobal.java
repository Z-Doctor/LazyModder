...
public class CustomRenderGlobal implements IWorldEventListener, IResourceManagerReloadListener {
	...
	/** The RenderEngine instance used by CustomRenderGlobal */
	...
	public List<CustomRenderGlobal.ContainerLocalRenderInformation> renderInfos = Lists.<CustomRenderGlobal.ContainerLocalRenderInformation>newArrayListWithCapacity(
			69696);
	/**
	 * Global tile entities, always rendered (beacon, end teleporter,
	 * structures)
	 */
	public final Set<TileEntity> setTileEntities = Sets.<TileEntity>newHashSet();
	public CustomViewFrustum viewFrustum;
	...
	public CustomRenderGlobal(Minecraft mcIn) {
		this.mc = mcIn;
	 ...
	 * CustomRenderGlobal.entityOutlineShader
	 */
	...
	public void loadRenderers() {
		if (this.world != null) {
			...
			this.viewFrustum = new CustomViewFrustum(this.world, this.mc.gameSettings.renderDistanceChunks,
					mc.renderGlobal, this.renderChunkFactory);
	...
	public void renderEntities(Entity renderViewEntity, ICamera camera, float partialTicks) {
		...
		if (this.renderEntitiesStartupCounter > 0) {
			...
			TileEntityRendererDispatcher.instance.prepare(this.world, this.mc.getTextureManager(), this.mc.fontRenderer,
					this.mc.getRenderViewEntity(), this.mc.objectMouseOver, partialTicks);
			this.renderManager.cacheActiveRenderInfo(this.world, this.mc.fontRenderer, this.mc.getRenderViewEntity(),
			...
			for (CustomRenderGlobal.ContainerLocalRenderInformation CustomRenderGlobal$containerlocalrenderinformation : this.renderInfos) {
				Chunk chunk = this.world.getChunkFromBlockCoords(
						CustomRenderGlobal$containerlocalrenderinformation.renderChunk.getPosition());
				ClassInheritanceMultiMap<Entity> classinheritancemultimap = chunk
						.getEntityLists()[CustomRenderGlobal$containerlocalrenderinformation.renderChunk.getPosition()
								.getY() / 16];
					...
					for (Entity entity2 : classinheritancemultimap) {
						...
						if (flag) {
							...
							if ((entity2 != this.mc.getRenderViewEntity() || this.mc.gameSettings.thirdPersonView != 0
									|| flag1)
									&& (entity2.posY < 0.0D || entity2.posY >= 256.0D || this.world
			...
			for (CustomRenderGlobal.ContainerLocalRenderInformation CustomRenderGlobal$containerlocalrenderinformation1 : this.renderInfos) {
				List<TileEntity> list3 = CustomRenderGlobal$containerlocalrenderinformation1.renderChunk
						.getCompiledChunk().getTileEntities();

				if (!list3.isEmpty()) {
					for (TileEntity tileentity2 : list3) {
						if (!tileentity2.shouldRenderInPass(pass)
								|| !camera.isBoundingBoxInFrustum(tileentity2.getRenderBoundingBox()))
							continue;
			...
			for (DestroyBlockProgress destroyblockprogress : this.damagedBlocks.values()) {
				...
				if (this.world.getBlockState(blockpos).getBlock().hasTileEntity()) {
					...
					if (tileentity1 != null && iblockstate.hasCustomBreakingProgress()) {
						TileEntityRendererDispatcher.instance.render(tileentity1, partialTicks,
	...
	public int getRenderedChunks() {
		...
		for (CustomRenderGlobal.ContainerLocalRenderInformation CustomRenderGlobal$containerlocalrenderinformation : this.renderInfos) {
			CompiledChunk compiledchunk = CustomRenderGlobal$containerlocalrenderinformation.renderChunk.compiledChunk;
	...
	public void setupTerrain(Entity viewEntity, double partialTicks, ICamera camera, int frameCount,
		...
		if (!flag && this.displayListEntitiesDirty) {
			...
			this.renderInfos = Lists.<CustomRenderGlobal.ContainerLocalRenderInformation>newArrayList();
			Queue<CustomRenderGlobal.ContainerLocalRenderInformation> queue = Queues.<CustomRenderGlobal.ContainerLocalRenderInformation>newArrayDeque();
			...
			if (renderchunk != null) {
				boolean flag2 = false;
				CustomRenderGlobal.ContainerLocalRenderInformation CustomRenderGlobal$containerlocalrenderinformation3 = new CustomRenderGlobal.ContainerLocalRenderInformation(
				...
				if (flag2 && !playerSpectator) {
					this.renderInfos.add(CustomRenderGlobal$containerlocalrenderinformation3);
				} else {
					...
					queue.add(CustomRenderGlobal$containerlocalrenderinformation3);
				}
			} else {
				...
				for (int j = -this.renderDistanceChunks; j <= this.renderDistanceChunks; ++j) {
					for (int k = -this.renderDistanceChunks; k <= this.renderDistanceChunks; ++k) {
						...
						if (renderchunk1 != null && camera.isBoundingBoxInFrustum(renderchunk1.boundingBox)) {
							...
							queue.add(new CustomRenderGlobal.ContainerLocalRenderInformation(renderchunk1,
									(EnumFacing) null, 0));
						}
					}
				}
			...
			while (!queue.isEmpty()) {
				CustomRenderGlobal.ContainerLocalRenderInformation CustomRenderGlobal$containerlocalrenderinformation1 = queue
						.poll();
				RenderChunk renderchunk3 = CustomRenderGlobal$containerlocalrenderinformation1.renderChunk;
				EnumFacing enumfacing2 = CustomRenderGlobal$containerlocalrenderinformation1.facing;
				this.renderInfos.add(CustomRenderGlobal$containerlocalrenderinformation1);

				for (EnumFacing enumfacing1 : EnumFacing.values()) {
					RenderChunk renderchunk2 = this.getRenderChunkOffset(blockpos, renderchunk3, enumfacing1);

					if ((!flag1 || !CustomRenderGlobal$containerlocalrenderinformation1
							.hasDirection(enumfacing1.getOpposite()))
						...
						CustomRenderGlobal.ContainerLocalRenderInformation CustomRenderGlobal$containerlocalrenderinformation = new CustomRenderGlobal.ContainerLocalRenderInformation(
								renderchunk2, enumfacing1,
								CustomRenderGlobal$containerlocalrenderinformation1.counter + 1);
						CustomRenderGlobal$containerlocalrenderinformation.setDirection(
								CustomRenderGlobal$containerlocalrenderinformation1.setFacing, enumfacing1);
						queue.add(CustomRenderGlobal$containerlocalrenderinformation);
					}
				}
			}
		...
		for (CustomRenderGlobal.ContainerLocalRenderInformation CustomRenderGlobal$containerlocalrenderinformation2 : this.renderInfos) {
			RenderChunk renderchunk4 = CustomRenderGlobal$containerlocalrenderinformation2.renderChunk;
	...
	public int renderBlockLayer(BlockRenderLayer blockLayerIn, double partialTicks, int pass, Entity entityIn) {
		...
		if (blockLayerIn == BlockRenderLayer.TRANSLUCENT) {
			...
			if (d0 * d0 + d1 * d1 + d2 * d2 > 1.0D) {
				...
				for (CustomRenderGlobal.ContainerLocalRenderInformation CustomRenderGlobal$containerlocalrenderinformation : this.renderInfos) {
					if (CustomRenderGlobal$containerlocalrenderinformation.renderChunk.compiledChunk
							.isLayerStarted(blockLayerIn) && k++ < 15) {
						this.renderDispatcher.updateTransparencyLater(
								CustomRenderGlobal$containerlocalrenderinformation.renderChunk);
					}
				}
			}
	...
	public void renderSky(float partialTicks, int pass, double x, double y, double z) {
		GlStateManager.pushMatrix();
...
//			tessellator.draw();
		...
		if (execute == 0 && movingObjectPositionIn.typeOfHit == RayTraceResult.Type.BLOCK) {
			...
			if (iblockstate.getMaterial() != Material.AIR && this.world.getWorldBorder().contains(blockpos)) {
				...
				drawSelectionBoundingBox(iblockstate.getSelectedBoundingBox(this.world, blockpos)
						.grow(0.0020000000949949026D).offset(-d3, -d4, -d5), 0.0F, 0.0F, 0.0F, 0.4F);
			}
	...
	public void setPartying(World p_193054_1_, BlockPos pos, boolean p_193054_3_) {
	...
	public void spawnParticle(EnumParticleTypes particleIn, double xCoord, double yCoord, double zCoord, double xSpeed,
	...
	@Nullable
	public Particle spawnEntityFX(int particleID, boolean ignoreRange, double xCoord, double yCoord, double zCoord,
			double xSpeed, double ySpeed, double zSpeed, int... parameters) {
		return this.spawnParticle0(particleID, ignoreRange, false, xCoord, yCoord, zCoord, xSpeed, ySpeed, zSpeed,
				parameters);
	}

	@Nullable
	public Particle spawnParticle0(int particleID, boolean ignoreRange, boolean minParticles, double xCoord,
		...
		if (this.mc != null && entity != null && this.mc.effectRenderer != null) {
			...
			if (ignoreRange) {
				return this.mc.effectRenderer.spawnEffectParticle(particleID, xCoord, yCoord, zCoord, xSpeed, ySpeed,
						zSpeed, parameters);
			} else if (d3 * d3 + d4 * d4 + d5 * d5 > 1024.0D) {
				return null;
			} else {
				return k1 > 1 ? null
						: this.mc.effectRenderer.spawnEffectParticle(particleID, xCoord, yCoord, zCoord, xSpeed, ySpeed,
								zSpeed, parameters);
			}
		} else {
			return null;
	...
	public int calculateParticleLevel(boolean p_190572_1_) {
	...
	public void broadcastSound(int soundID, BlockPos pos, int data) {
		...
		case 1038:
			...
			if (entity != null) {
				...
				if (soundID == 1023) {
					this.world.playSound(d7, d8, d9, SoundEvents.ENTITY_WITHER_SPAWN, SoundCategory.HOSTILE, 1.0F, 1.0F,
							false);
				} else if (soundID == 1038) {
					this.world.playSound(d7, d8, d9, SoundEvents.BLOCK_END_PORTAL_SPAWN, SoundCategory.HOSTILE, 1.0F,
							1.0F, false);
				} else {
					this.world.playSound(d7, d8, d9, SoundEvents.ENTITY_ENDERDRAGON_DEATH, SoundCategory.HOSTILE, 5.0F,
							1.0F, false);
				}
			}
	...
	public void playEvent(EntityPlayer player, int type, BlockPos blockPosIn, int data) {
		...
		switch (type) {
		case 1000:
			this.world.playSound(blockPosIn, SoundEvents.BLOCK_DISPENSER_DISPENSE, SoundCategory.BLOCKS, 1.0F, 1.0F,
					false);
			break;
		case 1001:
			...
			break;
		case 1002:
			this.world.playSound(blockPosIn, SoundEvents.BLOCK_DISPENSER_LAUNCH, SoundCategory.BLOCKS, 1.0F, 1.2F,
					false);
			break;
		case 1003:
			this.world.playSound(blockPosIn, SoundEvents.ENTITY_ENDEREYE_LAUNCH, SoundCategory.NEUTRAL, 1.0F, 1.2F,
					false);
			break;
		case 1004:
			this.world.playSound(blockPosIn, SoundEvents.ENTITY_FIREWORK_SHOOT, SoundCategory.NEUTRAL, 1.0F, 1.2F,
					false);
			break;
		case 1005:
			this.world.playSound(blockPosIn, SoundEvents.BLOCK_IRON_DOOR_OPEN, SoundCategory.BLOCKS, 1.0F,
					this.world.rand.nextFloat() * 0.1F + 0.9F, false);
			break;
		case 1006:
			this.world.playSound(blockPosIn, SoundEvents.BLOCK_WOODEN_DOOR_OPEN, SoundCategory.BLOCKS, 1.0F,
					this.world.rand.nextFloat() * 0.1F + 0.9F, false);
			break;
		case 1007:
			this.world.playSound(blockPosIn, SoundEvents.BLOCK_WOODEN_TRAPDOOR_OPEN, SoundCategory.BLOCKS, 1.0F,
					this.world.rand.nextFloat() * 0.1F + 0.9F, false);
			break;
		case 1008:
			this.world.playSound(blockPosIn, SoundEvents.BLOCK_FENCE_GATE_OPEN, SoundCategory.BLOCKS, 1.0F,
					this.world.rand.nextFloat() * 0.1F + 0.9F, false);
			break;
		case 1009:
			this.world.playSound(blockPosIn, SoundEvents.BLOCK_FIRE_EXTINGUISH, SoundCategory.BLOCKS, 0.5F,
		...
		case 1010:
			...
			break;
		case 1011:
			this.world.playSound(blockPosIn, SoundEvents.BLOCK_IRON_DOOR_CLOSE, SoundCategory.BLOCKS, 1.0F,
					this.world.rand.nextFloat() * 0.1F + 0.9F, false);
			break;
		case 1012:
			this.world.playSound(blockPosIn, SoundEvents.BLOCK_WOODEN_DOOR_CLOSE, SoundCategory.BLOCKS, 1.0F,
					this.world.rand.nextFloat() * 0.1F + 0.9F, false);
			break;
		case 1013:
			this.world.playSound(blockPosIn, SoundEvents.BLOCK_WOODEN_TRAPDOOR_CLOSE, SoundCategory.BLOCKS, 1.0F,
					this.world.rand.nextFloat() * 0.1F + 0.9F, false);
			break;
		case 1014:
			this.world.playSound(blockPosIn, SoundEvents.BLOCK_FENCE_GATE_CLOSE, SoundCategory.BLOCKS, 1.0F,
					this.world.rand.nextFloat() * 0.1F + 0.9F, false);
			break;
		case 1015:
			this.world.playSound(blockPosIn, SoundEvents.ENTITY_GHAST_WARN, SoundCategory.HOSTILE, 10.0F,
					(random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F, false);
			break;
		case 1016:
			this.world.playSound(blockPosIn, SoundEvents.ENTITY_GHAST_SHOOT, SoundCategory.HOSTILE, 10.0F,
					(random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F, false);
			break;
		case 1017:
			this.world.playSound(blockPosIn, SoundEvents.ENTITY_ENDERDRAGON_SHOOT, SoundCategory.HOSTILE, 10.0F,
					(random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F, false);
			break;
		case 1018:
			this.world.playSound(blockPosIn, SoundEvents.ENTITY_BLAZE_SHOOT, SoundCategory.HOSTILE, 2.0F,
					(random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F, false);
			break;
		case 1019:
			this.world.playSound(blockPosIn, SoundEvents.ENTITY_ZOMBIE_ATTACK_DOOR_WOOD, SoundCategory.HOSTILE, 2.0F,
					(random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F, false);
			break;
		case 1020:
			this.world.playSound(blockPosIn, SoundEvents.ENTITY_ZOMBIE_ATTACK_IRON_DOOR, SoundCategory.HOSTILE, 2.0F,
					(random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F, false);
			break;
		case 1021:
			this.world.playSound(blockPosIn, SoundEvents.ENTITY_ZOMBIE_BREAK_DOOR_WOOD, SoundCategory.HOSTILE, 2.0F,
					(random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F, false);
			break;
		case 1022:
			this.world.playSound(blockPosIn, SoundEvents.ENTITY_WITHER_BREAK_BLOCK, SoundCategory.HOSTILE, 2.0F,
					(random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F, false);
			break;
		case 1024:
			this.world.playSound(blockPosIn, SoundEvents.ENTITY_WITHER_SHOOT, SoundCategory.HOSTILE, 2.0F,
					(random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F, false);
			break;
		case 1025:
			this.world.playSound(blockPosIn, SoundEvents.ENTITY_BAT_TAKEOFF, SoundCategory.NEUTRAL, 0.05F,
					(random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F, false);
			break;
		case 1026:
			this.world.playSound(blockPosIn, SoundEvents.ENTITY_ZOMBIE_INFECT, SoundCategory.HOSTILE, 2.0F,
					(random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F, false);
			break;
		case 1027:
			this.world.playSound(blockPosIn, SoundEvents.ENTITY_ZOMBIE_VILLAGER_CONVERTED, SoundCategory.NEUTRAL, 2.0F,
					(random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F, false);
			break;
		case 1029:
			this.world.playSound(blockPosIn, SoundEvents.BLOCK_ANVIL_DESTROY, SoundCategory.BLOCKS, 1.0F,
					this.world.rand.nextFloat() * 0.1F + 0.9F, false);
			break;
		case 1030:
			this.world.playSound(blockPosIn, SoundEvents.BLOCK_ANVIL_USE, SoundCategory.BLOCKS, 1.0F,
					this.world.rand.nextFloat() * 0.1F + 0.9F, false);
			break;
		case 1031:
			this.world.playSound(blockPosIn, SoundEvents.BLOCK_ANVIL_LAND, SoundCategory.BLOCKS, 0.3F,
					this.world.rand.nextFloat() * 0.1F + 0.9F, false);
			break;
		case 1032:
			...
			break;
		case 1033:
			this.world.playSound(blockPosIn, SoundEvents.BLOCK_CHORUS_FLOWER_GROW, SoundCategory.BLOCKS, 1.0F, 1.0F,
					false);
			break;
		case 1034:
			this.world.playSound(blockPosIn, SoundEvents.BLOCK_CHORUS_FLOWER_DEATH, SoundCategory.BLOCKS, 1.0F, 1.0F,
					false);
			break;
		case 1035:
			this.world.playSound(blockPosIn, SoundEvents.BLOCK_BREWING_STAND_BREW, SoundCategory.BLOCKS, 1.0F, 1.0F,
					false);
			break;
		case 1036:
			this.world.playSound(blockPosIn, SoundEvents.BLOCK_IRON_TRAPDOOR_CLOSE, SoundCategory.BLOCKS, 1.0F,
					this.world.rand.nextFloat() * 0.1F + 0.9F, false);
			break;
		case 1037:
			this.world.playSound(blockPosIn, SoundEvents.BLOCK_IRON_TRAPDOOR_OPEN, SoundCategory.BLOCKS, 1.0F,
					this.world.rand.nextFloat() * 0.1F + 0.9F, false);
			break;
		case 2000:
			int j2 = data % 3 - 1;
		...
		case 2001:
			...
			if (block.getDefaultState().getMaterial() != Material.AIR) {
				...
				this.world.playSound(blockPosIn, soundtype.getBreakSound(), SoundCategory.BLOCKS,
		...
		case 2007:
			...
			this.world.playSound(blockPosIn, SoundEvents.ENTITY_SPLASH_POTION_BREAK, SoundCategory.NEUTRAL, 1.0F,
					this.world.rand.nextFloat() * 0.1F + 0.9F, false);
			break;
		case 2003:
		...
		case 2006:
			...
			this.world.playSound(blockPosIn, SoundEvents.ENTITY_ENDERDRAGON_FIREBALL_EPLD, SoundCategory.HOSTILE, 1.0F,
					this.world.rand.nextFloat() * 0.1F + 0.9F, false);
			break;
		case 3000:
			this.world.spawnParticle(EnumParticleTypes.EXPLOSION_HUGE, true, (double) blockPosIn.getX() + 0.5D,
					(double) blockPosIn.getY() + 0.5D, (double) blockPosIn.getZ() + 0.5D, 0.0D, 0.0D, 0.0D, new int[0]);
			this.world.playSound(blockPosIn, SoundEvents.BLOCK_END_GATEWAY_SPAWN, SoundCategory.BLOCKS, 10.0F,
					(1.0F + (this.world.rand.nextFloat() - this.world.rand.nextFloat()) * 0.2F) * 0.7F, false);
			break;
		case 3001:
			this.world.playSound(blockPosIn, SoundEvents.ENTITY_ENDERDRAGON_GROWL, SoundCategory.HOSTILE, 64.0F,
	...
	class ContainerLocalRenderInformation {
		...
		public ContainerLocalRenderInformation(RenderChunk renderChunkIn, EnumFacing facingIn,
	...
	public void renderSky(TileEntity te, double x, double y, double z, float partialTicks, int destroyStage,
			float alpha) {
		int pass = 0;
		net.minecraftforge.client.IRenderHandler renderer = this.world.provider.getSkyRenderer();
		if (renderer != null) {
